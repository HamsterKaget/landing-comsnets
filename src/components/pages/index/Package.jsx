        ...
        <div className="container h-auto mx-auto p-4">
          <div className="my-8">
            <div className="md:w-8/12 mx-auto text-center">
              ...
              <p className="text-slate-500 mt-2 mb-6">Tailored packages to suit your needs.</p>
            </div>
          </div>

          <div className="w-[95%] lg:w-[75%] mx-auto grid lg:grid-cols-3 sm:grid-cols-2 gap-12 lg:gap-8 py-12">
            {pricing.map((plan) => (
              <div key={plan.title} className="relative border border-slate-200 dark:border-indigo-500 dark:bg-slate-700 rounded-2xl dark:text-white shadow-card2 p-8 text-left flex flex-col hover:-translate-y-6 transition-all duration-500">
                ...
                <div className="mt-4 bg-slate-50 dark:bg-slate-600 p-6 rounded-lg -mx-6">
                  <p className="text-sm font-semibold text-slate-500 dark:text-indigo-50 flex items-center">
                    ...
                    <span className="ml-auto">{plan.frequency}</span>
                  </p>
                </div>
                <hr />
                <div className="-top-10 bg-slate-50 dark:bg-slate-600 p-6 rounded-lg -mx-6">
                  <p className="text-sm font-semibold text-slate-500 dark:text-indigo-50 flex items-center">
                    <span>{plan.currency2}</span>
                    <span className="ml-3 text-4xl text-slate-900 dark:text-white">{plan.price2}</span>
                    <span className="ml-auto">{plan.frequency}</span>
                  </p>
                </div>
